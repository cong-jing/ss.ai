...
## Batch 3.5: Candidate / Memory Staging / Memory Retained Layering

Detailed design draft: [memory-layered-refactor.zh-CN.md](memory-layered-refactor.zh-CN.md).
Batch 3.5 implementation plan: [memory-layered-refactor-batch3.5.zh-CN.md](memory-layered-refactor-batch3.5.zh-CN.md).
...
- `memory_staging`: processed, embedded, lightly de-duplicated intermediate memory fragments.
- `memory_retained`: stable, consolidated memories suitable for prompt injection.

High-level direction:

- Keep candidate intake simple: write pending rows and preserve source metadata.
- Move normalization, embedding, rule-based filtering, and lightweight duplicate handling into a candidate-to-memory-staging processor.
...
- Stop writing memory retained directly from candidate processing.
- Add memory staging storage and debug listing.
- Do not implement memory-staging-to-memory-retained processing or LLM judge in this batch.
...
- Pull unprocessed memory staging records.
- Retrieve related memory retained records.
...
- Mark processed memory staging records so they are not repeatedly considered.
