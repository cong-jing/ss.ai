...
/**
 * Error codes the adapter surfaces. Callers (commit service, debug API) match
 * on `code` to decide between retry, alert, or fail-soft.
 */
export type ModelClientEmbeddingProviderErrorCode =
    | "assignment_missing"
    | "api_key_missing"
    | "embed_unsupported"
    | "embed_failed"
...
export class ModelClientEmbeddingProviderError extends Error {
    public readonly code: ModelClientEmbeddingProviderErrorCode;

    constructor(code: ModelClientEmbeddingProviderErrorCode, message: string, options?: ErrorOptions) {
        super(message, options);
        this.name = "ModelClientEmbeddingProviderError";
        this.code = code;
    }
}
...
export interface ModelClientEmbeddingProviderDeps {
    modelClient: ModelClient;
    appStores: AppStores;
    defaultModelAssignments?: ModelAssignmentMap;
    /**
     * Provider name (lowercased) → encrypted API key. Used as a fallback when
     * the user has not supplied their own credential for the resolved
     * provider. Mirrors the structure consumed by `ModelRuntime`.
     */
    defaultProviderApiKeys?: Record<string, string>;
    /**
     * Embedding "signature" version covering NON-model factors: text
     * preprocessing, prompt wrapping, normalization strategy. Bump when any of
     * those change in a way that should invalidate stored vectors. Defaults
     * to 1.
     */
 ...
 *  2. Otherwise fall back to `defaultModelAssignments["memory.embed"]`.
 *  3. Otherwise throw `assignment_missing`.
 *
 * API key resolution mirrors `ModelRuntime`: per-user credential first, then
 * `defaultProviderApiKeys[provider]`. Missing key → `api_key_missing`.
 */
export class ModelClientEmbeddingProvider implements MemoryEmbeddingProvider {
    private readonly logger: PersonaFlowLogger;
    private readonly embeddingVersion: number;

    constructor(private readonly deps: ModelClientEmbeddingProviderDeps) {
    ...
    async embed(input: MemoryEmbedInput): Promise<MemoryEmbedResult> {
        ...
        if (text.length === 0) {
            // Caller (commit service) is expected to skip low-value drafts
            // before embedding. Treat empty as a programming error rather
            // than silently embedding "".
            throw new ModelClientEmbeddingProviderError(
                "embed_failed",
