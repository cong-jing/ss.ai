...
const validLogLevels: LogLevel[] = ["verbose", "debug", "info", "warn", "error"];
...
export interface ParseLogLevelOptions {
    fallbackLevel?: LogLevel;
    strict?: boolean;
    onInvalid?: (rawValue: string | undefined, fallbackLevel: LogLevel) => void;
}
...
export class Logger {
    ...
    verbose(message: string, meta?: unknown, onceOptions?: LoggerOverrideOptions): void {
        this.log("verbose", message, meta, onceOptions);
    }

    debug(message: string, meta?: unknown, onceOptions?: LoggerOverrideOptions): void {
        this.log("debug", message, meta, onceOptions);
    }

    info(message: string, meta?: unknown, onceOptions?: LoggerOverrideOptions): void {
        this.log("info", message, meta, onceOptions);
    }

    warn(message: string, meta?: unknown, onceOptions?: LoggerOverrideOptions): void {
        this.log("warn", message, meta, onceOptions);
    }

    error(message: string, meta?: unknown, onceOptions?: LoggerOverrideOptions): void {
    ...
    private log(
        ...
        const block = this.formatMessage(level, message, normalizedMeta, options.stackLevel, options.includeSourceLocation, options.includeStackTrace);
...
export function normalizeLogMeta(value: unknown): Record<string, unknown> | undefined {
    if (value === undefined) {
        return undefined;
    }
...
export function parseLogLevel(rawValue: string | undefined, options?: ParseLogLevelOptions): LogLevel {
    const fallbackLevel = options?.fallbackLevel ?? "info";
    const normalized = (rawValue ?? "").trim().toLowerCase();

    if (validLogLevels.includes(normalized as LogLevel)) {
        return normalized as LogLevel;
    }

    if (options?.strict) {
        const valueText = rawValue === undefined ? "undefined" : `\"${rawValue}\"`;
        throw new Error(`Invalid LOG_LEVEL: ${valueText}. Valid values: ${validLogLevels.join(", ")}.`);
    }

    options?.onInvalid?.(rawValue, fallbackLevel);
